...
class Currency(object):
    ...
    def sub_ok(self,k=1,b=0):
        tradeStr = """{"op": "subscribe", "args": ["spot/candle60s:"""+self.name+"""-USDT"]}"""
        ...
        while 1:
            ...
            if data_list :
                ...
                result["open"] = float('%.4f'%(float(data['candle'][1])*k+b+c))*2
                result["high"] = float('%.4f'%(float(data['candle'][2])*k+b+c))*2
                result["low"] = float('%.4f'%(float(data['candle'][3])*k+b+c))*2
                result["close"] = float('%.4f'%(float(data['candle'][4])*k+b+c))*2
    ...
    def sub_huobi(self,k=1,b=0):
        tradeStr = """{"sub": "market.""" + self.name.lower() + """usdt.kline.1min","id": "id1"}"""
        ...
        while (1):
            ...
            if 'ping' in result_dic.keys():
                ...
                if 'status' in result_dic.keys():
                    ...
                    c=self.get_riskNum()
                    result_dic["vol"] = result_dic.pop("amount")/1000
                    result_dic['open']=float('%.4f'%(result_dic['open']* k + b + c))*2
                    result_dic['high']=float('%.4f'%(result_dic['high']* k + b + c))*2
                    result_dic['low']=float('%.4f'%(result_dic['low']* k + b + c))*2
                    result_dic['close']=float('%.4f'%(result_dic['close']* k + b + c))*2
