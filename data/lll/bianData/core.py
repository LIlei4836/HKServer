...
class Currency(object):
    ...
    def save_data(self):
        ...
        if self.riskUrl:
            r.set('sub:' + self.projectName + ':' + sub_name + ':1min:bian', self.data)
            print('sub:' + self.projectName + ':' + sub_name + ':1min:bian')
        else:
            r.set('sub:' + sub_name + ':1min:bian', self.data)
            # print(('sub:' + sub_name + ':1min:bian'))
            print(sub_name, r.get('sub:' + sub_name + ':1min:bian'))
        ...
        while (1):
            ...
            if 'ping' in result_dic.keys():
                ...
                if 'status' in result_dic.keys():
                    ...
                    c=self.get_riskNum()
                    result_dic["vol"] = result_dic.pop("amount")
